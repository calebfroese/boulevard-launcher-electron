...
function registerListener(session, options, cb = () => {}) {
  ...
  const listener = (e, item, webContents) => {
    ...
    item.on('updated', () => {
      ...
      const bytesDiff = receivedBytes - prevBytes;
      const secondsDiff = (Date.now() - prevTime) / 1000;
      bytesPerSecond = bytesDiff / secondsDiff;
      ...
      if (!win.isDestroyed()) {
        win.setProgressBar(progressDownloadItems().progress);
      }
